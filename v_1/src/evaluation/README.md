...
cp archibab_nor.csv       v_1/data/raw/chungrong/archibab_nor.csv
cp oracc_let_adm_nor.csv  v_1/data/raw/chungrong/oracc_let_adm_nor.csv
cp lbl_nor.csv            v_1/data/raw/chungrong/lbl_nor.csv

# 2. Build unified corpus (merges 3 groups, filters ORACC to letters only)
python v_1/src/evaluation/01_create_corpus.py

# 3. Reconstruct texts + domain cleanup → ready for LLM evaluation
python v_1/src/evaluation/02_prepare_texts.py

# 4. (Optional) Clear old model cache if data changed significantly
rm v_1/data/evaluation/baselines/cache/*.jsonl

# 5. Set API key and run baseline
export OPENROUTER_API_KEY="your-key"
python v_1/src/evaluation/03_llm_baseline.py --model gpt-oss-20b --dry-run  # check first
python v_1/src/evaluation/03_llm_baseline.py --model gpt-oss-20b            # run
```
...
└── lbl_nor.csv                         ← Source: Late Babylonian (~600 BCE)
        │
        ▼
[01_create_corpus.py]                   ← Step 1: Merge + filter to letters only
        │
        ▼
...
├── unified_3groups_akkadian_letters.csv       ← Same, CSV format
        │
        ▼
[02_prepare_texts.py]                   ← Step 2: Reconstruct texts + domain cleanup
        │
        ▼
...
├── texts_token_stats.json              ← Token count estimates
        │
        ▼
[03_llm_baseline.py]                    ← Step 3: Call LLM API (per model)
        │
        ▼
v_1/data/evaluation/baselines/cache/
│   ├── gpt-oss-20b.jsonl              ← Cached predictions (resume-safe)
│   ├── llama-4-maverick.jsonl
│   └── ...
        │
        ▼
[04_aggregate_results.py]               ← Step 4: Merge all model predictions
        │
        ▼
v_1/data/evaluation/baselines/
├── baseline_predictions.parquet        ← All predictions in one file
        │
        ▼
[05_evaluate_baseline.py]               ← Step 5: Compute metrics + report
        │
        ▼
...
### Source Data (`v_1/data/raw/chungrong/`)
...
### Step 1: `v_1/src/evaluation/01_create_corpus.py`

**Input:** 3 source CSVs from `v_1/data/raw/chungrong/`
**Output:** `v_1/data/evaluation/corpora/unified_3groups_akkadian_letters.parquet` (+ CSV)
...
### Step 2: `v_1/src/evaluation/02_prepare_texts.py`

**Input:** `v_1/data/evaluation/corpora/unified_3groups_akkadian_letters.parquet`
...
### Step 3: `v_1/src/evaluation/03_llm_baseline.py`

**Input:** `v_1/data/evaluation/corpora/texts_for_evaluation.jsonl`
**Output:** `v_1/data/evaluation/baselines/cache/<model-name>.jsonl`
...
python v_1/src/evaluation/03_llm_baseline.py --model llama-4-maverick
```
...
```bash
python v_1/src/evaluation/04_aggregate_results.py  # merge all model caches
python v_1/src/evaluation/05_evaluate_baseline.py   # compute accuracy, F1, confusion matrices
```
...
Run it after `02_prepare_texts.py` to inspect the corpus visually.
...
Re-run steps 1 and 2. Both parquet and JSONL are regenerated from scratch by `02_prepare_texts.py`.
...
The JSONL must be generated by the current `02_prepare_texts.py` (which writes all fields). Re-run step 2.
