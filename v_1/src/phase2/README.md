...
- [x] F24 (23937) — the fallback self-labels via decoder-row lens: **the layer-9 year-ρ features are WHO-features** — German-surname morphology (44713), Chinese surnames (56768), Germany (2343), classical-Chinese dynastic register (9763: 帝王/天下/子孙), honorifics (50848); essentially zero calendar vocabulary, vs the late-layer cell-A direction's BC/BCE/ancient/公元前 (F6). Entity-mediated time restated at the feature level; explains F18 (firing without chronology). Tables in sae2/RESULTS.md.
- [ ] F23 interventions: 23761/23899/23921 all crashed on an empty control pool (controls drawn from the top-|ρ|-only hunt CSV); fixed on main (pool recomputed from encodings) — **STILL NEEDS ONE RESUBMISSION.** Amplify/ablate with rate-matched controls + THE BRIDGE. F12-null caution attached: null-with-control is the publishable outcome.
