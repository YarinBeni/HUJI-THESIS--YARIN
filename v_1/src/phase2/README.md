...
- [ ] F23 interventions: 23761/23899/23921 all crashed on an empty control pool — controls were drawn from the top-|ρ|-only hunt CSV. Fixed (pool recomputed from encodings) + scalar-threshold hardening; labels unblocked too (Cloudflare rejected python's User-Agent; confirmed source id `{L}-resid-batchtopk-65k__l0-80`). **Resubmit F23 once + run fetch_labels.** Amplify/ablate with rate-matched controls + THE BRIDGE. F12-null caution attached: null-with-control is the publishable outcome.
