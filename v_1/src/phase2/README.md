...
- [ ] F23 interventions: 23761/23899/23921 crashed on an empty control pool (fixed: pool recomputed from encodings). Review then caught a **bridge padding bug** before any results landed: right-padded shorter sequences were clamped at their readout position (`slice(0,T-1)` spares only the batch-longest), trivially inflating fire_last — fixed with a per-sample length mask. 23946 started pre-fix → **scancel + one resubmission**. Amplify/ablate with rate-matched controls + THE BRIDGE. F12-null caution attached: null-with-control is the publishable outcome.
- [ ] F25 feature interp (23945): max-activating contexts + Golden-Gate clamped generation — the primary feature-interpretation instrument (F24 lens downgraded to suggestive). Methodology + audit trail with references in sae2/RESULTS.md.
