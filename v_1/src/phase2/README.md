...
- [ ] F23 interventions: run 1 (23761) crashed without committing (log on cluster: sae2/logs/F23_fsteer_23761.out); prime suspect fixed (scalar batch-TopK threshold indexed per-feature). Rerun 23899 started before the fix synced — if it died too, resubmit once. Amplify/ablate with rate-matched controls + THE BRIDGE. F12-null caution attached: null-with-control is the publishable outcome.
