...
git add \
    v_1/src/linear_probing/results/orcc__embed/orcc_qwen_coords_mean.json \
    v_1/src/linear_probing/results/orcc__embed/orcc_qwen_coords_last.json
git commit -m "Add ORCC coord JSONs: mean+last t-SNE/PCA/UMAP (cluster job $SLURM_JOB_ID)" \
    || echo "Nothing new to commit"
git push origin main \
    || echo "WARNING: git push failed — results saved locally at results/orcc__embed/"
