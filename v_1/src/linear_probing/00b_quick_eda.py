...
Extract final-layer mean-pooled embeddings for all 4,957 texts,
run PCA + t-SNE (skip UMAP if not installed), save plot and embeddings.
"""
...
from utils import (
    load_letters, clean_tier0, model_short_name, mean_pool,
...
def run(args):
    ...
    print(f"Extracting final-layer embeddings in {n_batches} batches...")
    ...
    with torch.no_grad():
        for i in range(0, len(texts), batch_size):
            ...
            pooled = mean_pool(last_hidden, attention_mask)  # (batch, hidden_dim)
    ...
    embeddings = np.concatenate(all_embeddings, axis=0)  # (4957, hidden_dim)
    print(f"Embeddings shape: {embeddings.shape}")
    assert embeddings.shape[0] == len(df), f"Expected {len(df)} rows, got {embeddings.shape[0]}"
    assert not np.any(np.isnan(embeddings)), "NaN in embeddings!"

    # ── Save embeddings ─────────────────────────────────────────────────────
    act_dir = RESULTS_DIR / 'activations' / short_name
    act_dir.mkdir(parents=True, exist_ok=True)
    emb_path = act_dir / 'final_layer_only.npz'
    ...
    fig.suptitle(f'{short_name} — Final-Layer Embeddings (mean-pooled, tier0 clean)',
                 fontsize=14, y=1.02)
    plt.tight_layout()

    plots_dir = RESULTS_DIR / 'plots'
    plots_dir.mkdir(parents=True, exist_ok=True)
    plot_path = plots_dir / 'quick_eda_final_layer.png'
