...
SPINE = [
    ...
    ("new", "b_frag_year", "Whole fragments in English: a bag of n-grams dates them best"),
...
def _fix_p9(sec):
    ...
    sec = sec.replace('G-KPLS &le; RBF-KPLS in 30/34 cells', 'G-KPLS &le; RBF-KPLS in 36 of 44 cells')
...
"matrix": """<section class="slide slide-text">
  ...
  <table class="rtbl matrix"><thead><tr><th></th><th>High-resource language (English)</th><th>Low-resource language (Akkadian)</th></tr></thead><tbody>
    ...
    <tr><td><span class="mdl">Obscure entities</span></td><td><strong>B:</strong> Assyrian rulers and find-spots, written in English. <em>Slides 9&ndash;12.</em></td><td><strong>C:</strong> the same entities in raw Akkadian. <em>Slides 13&ndash;20.</em></td></tr>
  </tbody></table>
...
"protocol": """<section class="slide slide-text">
  ...
  <div class="cfg">
    <div class="cfg-k">Models</div><div class="cfg-v">Llama-2 <strong>7B / 13B / 70B</strong> (the paper's own series) &middot; Qwen3 <strong>1.7B / 8B / 32B</strong> &middot; <strong>gpt-oss-120B</strong> &middot; three small seq2seq encoders from the translation line: <strong>cuneiform-400M</strong> and <strong>AKK-300M</strong> (translation-finetuned) and <strong>uMT5-base</strong> (their multilingual base, no translation finetune).</div>
    <div class="cfg-k">Controls</div><div class="cfg-v"><strong>random-initialised twins</strong> for four of the decoders, all three Llama-2 sizes and Qwen3-8B (same architecture and tokenizer, untrained weights), and a <strong>TF-IDF n-gram floor</strong> built from word and character n-grams. Both appear as rows in every results table, never as a slide of their own.</div>
    <div class="cfg-k">Pooling</div><div class="cfg-v"><strong>entity last token</strong> (the paper's read-out, an explicitly marked span) &middot; <strong>text last token</strong> (their Headlines read-out, applied to our whole fragments) &middot; <strong>mean over the text</strong> (new: no span is marked, so the signal must be recoverable implicitly, and the read-out we report for the bidirectional encoders, whose final token is not a running summary the way a causal model's is).</div>
...
"cellA_repro": """<section class="slide slide-text">
  ...
  <div class="cfg tight">
    <div class="cfg-k">Setup</div><div class="cfg-v">the paper's six English datasets (World, USA, NYC &rarr; <strong>latitude/longitude</strong>; Figures, Art, Headlines &rarr; <strong>year</strong>). Each entity's last-token embedding for the decoders and the average over the entity's tokens for the bidirectional encoders, every layer, probed on held-out entities. We reproduce it with <strong>our extended set of models</strong> (both decoder families, gpt-oss-120B, three translation encoders) and with the <strong>controls the paper never ran</strong>.</div>
  ...
  {{TABLE:cellA}}
  <p class="fig-note"><strong>On English the published result holds, and it holds against controls the paper never ran.</strong> Our Llama-2-70B lands within .03 of every published number, and the effect extends to a second family: Qwen3 scales 1.7B &rarr; 8B &rarr; 32B exactly as the paper's scaling claim predicts. The gap to the controls is large in <em>both</em> space and time (Llama-2-70B World .905 against its random twin .170; Art .860 against .029), so the geometry is learned rather than architectural, and TF-IDF is a genuine floor: every random arm falls below it, and the trained models clear it on five of the six datasets (NYC, the paper's own weakest, is the exception). The three <strong>translation models</strong> (uMT5-base, cuneiform-400M, AKK-300M) sit between the random twins and the decoders on English, usually below TF-IDF: they are not generically good probes. <strong>PLS tracks Ridge closely</strong> for every decoder. <em>* = control.</em></p>
</section>""",
...
"cellA_layers": """<section class="slide slide-figure fig-major">
  ...
  <div class="takeaway tight"><span class="tk-label">Key takeaway</span>The Qwen and Llama families sit clearly above both the random controls and the encoder-decoder translation models. <strong>Where each arm peaks is as informative as how high it peaks:</strong> the random controls peak at the edges of the network, usually the very first layers, under last-token R&sup2; sometimes the very last, which is what "no signal was built along the way" looks like, whereas every arm that beats its controls peaks in the <strong>middle-to-late</strong> layers, reproducing the paper's depth profile for time under both poolings. In space the best layers are spread more evenly across depth and sit less often at the very start. One arm is the exception worth watching in the space panels: the Llama-2-70B random twin tracks near the bottom for most of the network under both poolings and then <strong>rises sharply in the last few layers</strong>, and two smaller random twins do the same under last-token pooling.</div>
</section>""",
...
"cellA_pls": """<section class="slide slide-figure fig-major">
  ...
  <div class="takeaway tight"><span class="tk-label">Key takeaway</span>Year concentrates hard: every decoder's best k is exactly <strong>16</strong> in all four time panels. Space spreads higher, k &asymp; 16 to 64 depending on the arm. That is intuitively what the strong scores require: with only a handful of directions there would be no room for a meaningful subspace, and we would expect the weak results the <strong>random-init controls</strong> show, which saturate by k &asymp; 3 to 5 on time, and on space creep along to k &asymp; 16 to 32 without ever approaching the trained arms. The learned representation is genuinely multi-dimensional rather than one strong axis, a distinction that a single best-layer R&sup2; table hides completely.</div>
</section>""",
  ...
  <div class="two-col">
    <div>
      <div class="text-points">
        ...
        <div class="tp"><div class="tp-h">The objective, and where it comes from</div><div class="tp-b"><strong>Masked language modelling</strong> rather than next-token prediction, following <a href="https://arxiv.org/abs/2109.04513"><em>Filling the Gaps in Ancient Akkadian Texts</em></a> (Lazar et al., EMNLP 2021), which showed that restoring a broken tablet <em>is</em> the masked-token task and that bidirectional context matters when neighbouring signs are damaged. The same instinct runs through <a href="https://www.nature.com/articles/s41586-022-04448-z">Ithaca</a> (Assael et al., Nature 2022) and <a href="https://www.nature.com/articles/s41586-025-09292-5">Aeneas</a> (Assael et al., Nature 2025) for Greek and Latin.</div></div>
        <div class="tp"><div class="tp-h">Architecture</div><div class="tp-b">A <strong>16-layer pre-norm transformer encoder</strong>, 37M parameters: d = 384, feed-forward 1536, 8 heads, RoPE positions, RMSNorm, MLM head over the sign vocabulary (15&#37; masking, 80/10/10). Trained 10 epochs; validation loss 4.55 &rarr; 3.02. The figure shows the full flow, with two deliberate differences from the Ithaca design it follows: a single <strong>sign-level</strong> input row (no word row), and a <strong>single restoration head</strong>, the region and date heads being replaced by linear probes on the frozen per-layer activations.</div></div>
...
"c_kingtoken": """<section class="slide slide-text">
  ...
  <div class="cfg tight">
    ...
    <div class="cfg-k">Cleaning</div><div class="cfg-v">the fragment column uses our <strong>maximal</strong> regime, an eleven-filter pipeline, one of whose filters truncates to the first 30 words. It strips digits, logograms (all-capital tokens), determinatives, case endings and plural markers, normalises long vowels, and lowercases everything. The reason is measured, not stylistic: without it a bag of n-grams reaches 99&#37; accuracy by reading <em>document length and royal-name spellings</em>, because well-preserved eras leave long inscriptions and poorly-preserved ones leave scraps. Truncation removes the length crutch, the filters remove the name crutch. The name column deliberately keeps the names, since that is what it is measuring.</div>
    ...
    <div class="cfg-k">Metric</div><div class="cfg-v"><strong>Spearman &rho;</strong>, because the target is nearly constant per king (seven of the 8 rulers carry a single year) and dating is a ranking problem; R&sup2; is unstable at that granularity and is reported for the whole-fragment tables instead. Averaged over <strong>200 balanced draws</strong> of 8 rulers x 21 fragments, grouped by ruler inside every draw; the name column keeps only the fragments in each draw where the ruler's name actually appears, so its draws are smaller. Each cell is <strong>PLS</strong>|<span style="color:#6b7484">Ridge</span>; a shuffled-label null sits at about 0.01.</div>
  </div>
...
"ruler_not_chrono": """<section class="slide slide-text">
  ...
  <div class="cfg tight">
    <div class="cfg-k">The question</div><div class="cfg-v">the previous slide showed the ruler's name token scoring far above the whole fragment. Before reading that as chronology, we ask what the probe is actually doing there. The same name-token read-out, on its own balanced draws over the fragments where a royal name survives the cleaning, with two questions: <strong>can it name the ruler</strong>, and <strong>can it order the years</strong>.</div>
    <div class="cfg-k">Metrics</div><div class="cfg-v"><strong>Ruler identification</strong> is macro-F1 over the <strong>5 rulers</strong> whose names appear often enough to balance, so chance is .20. <strong>Year ordering</strong> is Spearman &rho; against the true years. Both on the raw Akkadian, names left in, since the name is the thing being read.</div>
  </div>
  ...
  <div class="takeaway tight"><span class="tk-label">Key takeaway</span><strong>An untrained network scores .946 and .926.</strong> A randomly initialised Qwen3-8B identifies the five kings almost perfectly and orders their years almost perfectly, matching every trained model in the table and edging one of them. Telling eight fixed spellings apart requires no learned history at all, and once you know which king wrote a text you know its year for free, because in this corpus almost every ruler carries a single date. So the name column is a <strong>lookup table, not a timeline</strong>: it measures whether the tokenizer preserved the name, and nothing else. <strong>This is why the rest of the deck reads the whole fragment with the names stripped</strong>, and it is the read-out under which the models stop separating from their controls.</div>
</section>""",
...
"c_frag_year": """<section class="slide slide-text">
  ...
  <div class="cfg tight">
    <div class="cfg-k">Task</div><div class="cfg-v">predict <strong>the year a fragment was written</strong>, exactly as on the English fragment slide, but from the <strong>Akkadian text itself</strong> rather than from its English translation. Nothing else changes, so the gap between this table and that one is the cost of the language alone.</div>
    <div class="cfg-k">Text</div><div class="cfg-v">cleaned, with royal names and other surface giveaways stripped and every fragment capped at its first 30 words, so a score cannot come from spotting a king's name, and the length shortcut, well-preserved periods leaving longer texts, is blunted.</div>
...
"ntp_finetune": """<section class="slide slide-figure fig-major">
  ...
  <div class="cfg tight">
    <div class="cfg-k">Setup</div><div class="cfg-v">we continued pretraining <strong>Qwen3-1.7B, Qwen3-8B, Qwen3-32B and gpt-oss-120B</strong> on our Akkadian fragments with ordinary <strong>next-token prediction</strong>, the objective they were built with, using the training split of the same corpus the MLM was trained on (the test fragments are never touched). Each model was fine-tuned at <strong>four unfreezing depths</strong>, from the whole network down to only the top tenth (the 120B through LoRA adapters rather than full weights), so the update could be placed where the probe actually reads. The tokenizer was left alone: the tokens worth adding are royal and divine names, which is exactly the leakage channel the cleaning removes.</div>
  ...
  <div class="takeaway tight"><span class="tk-label">Key takeaway</span><strong>Nothing moves.</strong> Across four models and four unfreezing depths the change is between &minus;.013 and +.002 &rho;, far inside the spread of the draws, and for the two largest models several arms are identical to base at this precision because the layers the probe reads were frozen (for gpt-oss-120B, bit-identical). More exposure to Akkadian, delivered through the objective these models were built with, does not create a temporal axis. The honest caveat is <strong>scale of data</strong>: our corpus is about 2.5M words, effectively all the published Akkadian available, five to six orders of magnitude below these models' pretraining. So the fair claim is not that more Akkadian could never help; it is that <em>effectively all the Akkadian there is to give</em>, used this way, does not.</div>
</section>""",
...
"shuffle": """<section class="slide slide-text">
  ...
  <div class="cfg tight">
    ...
    <div class="cfg-k">Target &amp; metric</div><div class="cfg-v"><strong>year</strong>, Spearman &rho;, average pooling, 200 balanced draws over the 8 rulers, best layer per arm, <strong>PLS</strong>; the Ridge companion is in the CSV and agrees on the small costs, though it flips sign on the largest one (Qwen3-8B), so read that single cell cautiously.</div>
  </div>
  <table class="rtbl compact"><thead><tr><th rowspan="2">model</th><th colspan="3" class="num">cleaned Akkadian</th><th colspan="3" class="num">English translation</th></tr><tr><th class="num">in order</th><th class="num">shuffled</th><th class="num">cost</th><th class="num">in order</th><th class="num">shuffled</th><th class="num">cost</th></tr></thead><tbody>
    ...
    <tr class="rand"><td><span class="mdl">TF-IDF</span> (Ridge)</td><td class="num">.266</td><td class="num">.266</td><td class="num">.000</td><td class="num">&ndash;</td><td class="num">&ndash;</td><td class="num">&ndash;</td></tr>
  </tbody></table>
...
"conditions": """<section class="slide slide-text">
  ...
  <div class="text-points">
    <div class="tp"><div class="tp-h">Condition 1: the language must be well represented in training</div><div class="tp-b">This is the load-bearing factor. Holding the entities fixed and moving from English to Akkadian drops every trained model <strong>onto its own untrained twin</strong>, while an n-gram baseline matches the best of them. The geometry the paper found does not transfer to a language the model barely saw.</div></div>
    ...
    <div class="tp"><div class="tp-h">Condition 3: the read-out must sit where the information is</div><div class="tp-b">Pooling was never a side detail. On bare names the paper's entity-last-token choice carries the result and averaging destroys it; on whole fragments the relation flips and averaging adds between +.07 and +.26 depending on the arm. A claim about what a model represents is inseparable from where you read it.</div></div>
    <div class="tp"><div class="tp-h">What does not substitute for any of them</div><div class="tp-b"><strong>Scale</strong> (flat from 1.7B to 120B on Akkadian, and gpt-oss-120B lands mid-pack even on English) &middot; <strong>declarative knowledge</strong> (the models recite these kings' reigns in English) &middot; <strong>prompting</strong>, <strong>asking directly</strong>, <strong>continued pretraining on effectively all the published Akkadian</strong>, <strong>word order</strong>, <strong>curved and kernel probes</strong>, <strong>unsupervised geometry</strong>. Every rescue leaves the numbers where they were.</div></div>
  </div>
</section>""",
...
"olmo_freq": """<section class="slide fig-major">
  ...
  <div class="takeaway tight"><span class="tk-label">Key takeaway</span><strong>Exposure is not the variable.</strong> Across a 40,000-fold range of training frequency the overall correlation with dating error is &rho;&nbsp;&minus;.04 (&minus;.10 within century); only the extremes separate, by ~34 years on a ~105-year task. The third panel is the same data with the log dropped: on a raw count axis the median sits between <strong>73 and 120 years in every bin</strong>, around an overall median of 103 &mdash; a flat line, so the log transform is not what produces it. Even people whose name <em>never appears once</em> are dated to ~122 years &mdash; and counting surnames too, which dissolves 86&#37; of that group, the 51 who remain at zero under both forms are still dated ~94 years better than the twin manages on anyone. The counts also overturn the salience axis itself: the "obscure" Assyrian rulers are <strong>as frequent in the corpus as the "salient" figures</strong> (median 1,494 vs 230 raw; 42nd percentile once name length is controlled), with Sennacherib at 449,892 occurrences. So the cell-A &rarr; cell-C collapse cannot be about how much the models saw of these entities &mdash; it tracks the <strong>form of the input</strong>: the step from an entity's name to a whole document is where the linear read-out dies, in both languages.</div>
</section>""",
...
def cellA_table():
    ...
    # The Ridge columns come from the aggregate summaries, whose canonical site
    # is last-token for decoders but MEAN for the bidirectional encoders. The
    # PLS value must be read from the same site (and hence the same best layer)
    # as the Ridge value it shares a cell with.
    enc = {"umt5_base", "thalesian_akk300m", "thalesian_cunei400m"}
    pls = {}
    pls_dir = os.path.join(WM, "results", "eng_pls")
    for arm in os.listdir(pls_dir) if os.path.isdir(pls_dir) else []:
        pls[arm] = {}
        site = "mean" if arm in enc else "last"
        for ds, _ in DATASETS:
            f = os.path.join(pls_dir, arm, f"{ds}.{site}.json")
