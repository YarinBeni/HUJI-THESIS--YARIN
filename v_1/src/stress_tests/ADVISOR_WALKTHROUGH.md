...
| model | kp0 date acc (±50yr) | kp1 period recall | kp2 halluc. rate (gate) |
|---|---|---|---|
| Qwen3-1.7B | 0.875 (7/8) | 0.50 | 0.75 (FAIL) |
| Qwen3-8B | 0.875 (7/8)* | 0.625 | 0.00 (pass) |
| Qwen3-32B | 0.75 (6/8) | 0.25 | 0.00 (pass) |
| gpt-oss-120B | 1.00 (8/8) | 0.00† | 0.00 (pass) |

*8B's one "miss" is a parse error (output truncated mid-JSON), not a wrong date — scoreable
accuracy is 8/8. †gpt-oss kp1=0 is a format artifact (its list format doesn't match the exact-match
scorer) — a scoring caveat, not zero knowledge.
...
| model (tier0) | best L | great-circle err | skill vs centroid |
|---|---|---|---|
| Qwen3-1.7B | 11 | 190 km | +0.287 |
| Qwen3-8B | 12 | 187 km | +0.298 |
| Qwen3-32B | 63 | 179 km | +0.326 |
| gpt-oss-120B | 10 | 189 km | +0.290 |
| Thalesian-AKK-300m | 7 | 192 km | +0.279 |
| **Thalesian-cunei-400m** | 11 | **172 km** | **+0.354** |
| uMT5-base | 9 | 203 km | +0.237 |
| random Qwen3-8B | 31 | 204 km | +0.232 |
...
- Build **153 anchor prompts** whose date we know, in English, on the model's *declarative* side:
  one per ruler ("an Akkadian royal inscription from the reign of {ruler}") + one per decade
  ("...from the year {year} BCE"). Embed each (mean-pooled) at every layer.
- **3a — do the anchors form a timeline?** Fit a 1-D embedding of the anchors — PCA-1D (linear)
  and Isomap-1D (nonlinear, cosine) — and take |Spearman(1-D coordinate, anchor year)|.
- **3b — do real texts land on it?** Each ORCC text (tier0, mean-pooled) predicts its year as the
  year of its **nearest anchor by cosine**; Spearman vs truth.
...
| model | best L | 3a (anchors line up) | 3b (texts project) |
|---|---|---|---|
| Qwen3-1.7B | 16 | 0.388 | 0.034 |
| Qwen3-8B | 21 | 0.381 | 0.060 |
| Qwen3-32B | 22 | 0.484 | 0.110 |
| gpt-oss-120B | 11 | **0.567** | 0.105 |
| Thalesian-AKK-300m | 0 | 0.250 | 0.088 |
| Thalesian-cunei-400m | 12 | 0.428 | 0.042 |
| uMT5-base | 10 | 0.206 | 0.134 |
| random Qwen3-8B | 28 | 0.342 | 0.040 |
...
| model | best macro-F1 | random model |
|---|---|---|
| Qwen3-1.7B | 0.720 | |
| Qwen3-8B | 0.691 | |
| Qwen3-32B | 0.717 | |
| gpt-oss-120B | 0.689 | |
| Thalesian-cunei-400m | 0.720 | |
| **random Qwen3-8B** | | **0.667** |
...
- Re-run the *identical* year probes on these prompted activations: GroupKFold AND balanced-MC
  (same 200 draws, mapped by fragment_id). Causal chat models only (Qwen3 1.7B/8B/32B — GKF and
  MC all complete; gpt-oss never ran on T10).
