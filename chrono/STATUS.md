...
| C3-1 | high | C3 (jobs 33318_*) died on `EmbStore missing 56976 ids ... L11`: `emin_thalesian.yaml` still carried the 12-block layer index | `layer: 8`; resubmitted via runner (006). Row-L2 gate rerun did NOT recover the M.Sc. .352 (akk PLS L8 .148) — recorded in `docs/gate_reference.md` as a finding about the M.Sc. mean-of-valid-folds read-out; ridge rows reproduce, C3 proceeds |
| C3-2 | **result** | **E-MIN ran (25/25 seed×fold).** Chrono-Barlow head on L8/mean: `orig` gkf-pooled ρ .317±.025, mc ρ .398±.022 (5 seeds) vs ridge .260/.287 and PLS .141/.126 on the SAME features/folds/read-out. Under `mask_ruler` the head keeps .370 (−.03); `mask_ruler,crop16` .320. Block null of the reported statistic .002±.013 (95% ≤ .03). identical_view_rate 2% (was 23–39%) | `scripts/aggregate_emin.py` → `reports/emin_summary.md`. Missing for the plan's claim: the BASELINE under the same masked conditions (ridge fit on orig, read on views) — next job |
| C3b-1 | high | baseline_conditions fitted ridge/PLS on DUPLICATED rows (orig chain × 2 view seeds, identical text) → RidgeCV LOO picked a memorising alpha; akk-only orig read .094 vs C2 .287 and masking names *helped* | training rows unique by text; rerun as runner job 008 (all-lang / akk / eng) |
| C3c | **result** | **E-MIN closed.** Head ≈ ridge on clean text (.398±.022 vs .376); under name-masking/cropping the head degrades least by +.02–.04 mc over ridge trained on all views; masking names barely hurts ANY method (−.01…−.04); PLS k=2 not a serious baseline (.165). Plan's '≤ half PLS degradation' criterion not met as written (PLS does not degrade) | `reports/EMIN_RESULT.md`. Next: `lang` in score schema, baseline seed spread, duplicate-text policy, then P1 ladder |
| V2 | plan | **E-MIN v2** after Yarin's review of E-MIN: (i) `maximal` is already a heavy cleaning — masking on top may leave nothing by construction → **tier0** Akkadian (1,193 docs, own root `chrono/artifacts_tier0`); (ii) AKK_300m is a weak encoder → **Llama-2-7B L16, Qwen3-8B L18** (the M.Sc.'s best LLMs on akk / gloss) **+ cuneiformBase-400m L12**; (iii) language arms **akk / eng / mix**, with per-language `<cond>@<lang>` conditions in the score schema | configs `emin2_*`, jobs `C1v2_extract` (3) → `C3v2_emin` (45, each seed-0 task also runs the gate + baselines-on-conditions for its arm); submitted via runner 010 |
| V2-1 | high | C1v2 (33341, all 3 tasks) died: EmbStore's `manifest.parquet` was read-modify-written **in place, unlocked** on every put; three concurrent extract tasks → readers saw half a file (`Couldn't deserialize thrift`). v1 never hit it because C1 had one writer | manifest write is `fcntl`-locked + temp-file + `os.replace`; shards now carry model/layer/site/text_sha so a corrupt manifest rebuilds from shards; 3 new tests (6-process writers, corrupt-manifest rebuild, no in-place write). Store wiped and both arrays resubmitted via runner 011 |
| V2-R | **result** | **E-MIN v2 closed.** Akkadian tier0, mc ρ: cunei400m head **.61±.01** vs ridge .45 / ridge-all-views .46 / PLS .44; Llama-2-7B **.54±.03** vs .35/.44/.22; Qwen3-8B **.43±.02** vs .26/.33/.20. Block null .00±.02. Head most robust to crop16 everywhere; name masking hurts nothing (3rd time). Akkadian > gloss with a competent encoder. Head loses on eng with the Akkadian-only encoder | `reports/EMIN2_RESULT.md`, `EMIN2_TABLES.md`. Next: P1 ladder on cunei400m + Llama tier0; baseline seed spread; duplicate/century policy |
| P1-1 | **result** | **Erasure ladder (probe level, C4).** Provenance erasure removes 65–100% of the linear dating signal (cunei .45→.14, Llama .36→.00, Qwen .26→−.20); period −.10…−.20; object type −.04…−.15; length ≤.10; controls crush | `reports/LADDER_RESULT.md`. Head ladder C5 (36 GPU tasks) submitted via runner 015; readability re-pass 33445 chained |
| P1-2 | **result** | **Head ladder (C5, 36 runs).** After provenance erasure ridge reads .14/.00/−.20 (cunei/Llama/Qwen), the head .36/.38/.29 (59–70% retained); margin +.22/+.38/+.49. Period −.15…−.25, object type −.03…−.14, length ≈0 for the head | `reports/HEAD_LADDER_RESULT.md`. Nonlinear-recovery check queued (C5b heads → probe_head_hidden) |
| P1-3 | **negative** | **Nonlinear-recovery check.** After LEACE, provenance is linearly at chance but an MLP reads it from erased X (.25/.35/.42); the head's hidden layer makes it *linearly* readable again on Llama/Qwen (.42/.46 vs raw .42/.41) → their post-erasure ρ is site reconstruction. cunei400m head does not reconstruct (lin .06, MLP .17) | `reports/HEAD_LADDER_RESULT.md` last section. Next: HSIC/adversarial deconfounding vs provenance in the objective |
| P2-1 | null | **HSIC deconfounding (C6, λ 1/10)** left provenance readability from h unchanged (.43/.41/.28 vs raw .44/.42/.41) and ρ unchanged; λ=1 ≡ λ=10 → penalty scale negligible vs Barlow term | `reports/HSIC_RESULT.md`. C6b: kernel CKA (scale-free) λ∈{1,5}, penalty logged as `final_dep`; runner 022/023 |
| P2-2 | closed | **CKA (C6b) also null** (provenance from h .42/.40/.29 with penalty active; ρ .58/.51/.43); adversary prototype unstable. **Line closed by Yarin**: method = text-view invariance; find-spot = stated limitation, not a training target | `reports/HSIC_RESULT.md` decision section. Uncommitted adversary code discarded |
| DATA-2 | note | **Duplicate texts across documents**: 136 akk docs in 50 groups of byte-identical text (44 eng / 17 groups) — ORACC exemplars of one inscription. 4 akk groups span >1 ruler. Within-ruler duplicates cannot leak across gkf folds; cross-ruler ones can, and they also inflate any per-doc count | not yet acted on; candidate for the corpus contract (dedupe or exemplar-group id) before P1 |
